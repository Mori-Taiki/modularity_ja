...
🫥 え？「選択して`Ctrl + F12`で飛べる」って？そんなのいちいち覚えてらんないよ…俺は"慣れた"手段で仕事がしたいんだ。
...
🫥え？「レートが0〜1の範囲内かどうか のチェックはレートという値自身が保証すべき」って？どこでチェックしても同じでしょ。

🫥何？まだ文句あるの？「それに、`TipRate` と `TaxRate` が型で区別できなくなった」「取り違えたときにコンパイラで気づけないだろ」…？そんなの変数名ちゃんとしてればそんなこと起こらないよ、人間を信じなって。
...
え？
> 「違うものを同じところにまとめるくらいなら別々の方がマシだった、どっちかがUtilを変更したらもう片方が引きずられてバグる」

🫥もう、まとめりゃいいのか別々にすりゃいいのかはっきりしてくれよ…

>「共通化と抽象化は違う、DRY原則は抽象化の話だ」…？「じゃあ何で値オブジェクト消したんだよ」
...
実際、彼の主張には **特定の条件下では正しいもの** がたくさんあります。問題は、彼が一貫して「このコードは変わらない」「この要件は増えない」という ***低変動性を暗黙の前提*** としていることです。
...
> 侵入結合はよほどのことが無い限り発生しないので、今回は例を省きました（🫥「InternalsVisibleToやリフレクションがマズいことくらい、弁えてるよ！」）その代わりに、Utilで間違った共通化を起こし、実質的に侵入結合と近しいリスクを抱える形式にしています。

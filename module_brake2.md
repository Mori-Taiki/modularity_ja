こんにちは、エンジニアのT.Mです。
本記事では Vlad Khononov 『ソフトウェア設計の結合バランス』に基づき、モジュール結合の4段階を Checkout 機能の同一仕様のコードで示してみようと思います。今回はあえて**逆リファクタリング**の形式をとってみます。
...
- 各ステップで小数第2位に丸める
- 割引率・税率は `0〜1` の範囲
- ダッシュボード等の利用側に向け、直近の `Receipt` を返す読み取りAPI（`GetRecent`）を公開する

変わるのは「**そのルールを誰が知っているか**」だけです。利用側に漏れる知識が増えるほど、結合は強くなります。

# 結合の4段階

Khononov の定義では、モジュール結合は共有される知識の量で4段階に分かれます。
「結合強度」とはKhononov独自の用語です。以下の表では、結合が弱い順に上からならべており、下になるほど結合が強固になります。既存の用語でいえば、上であるほど疎結合で、下ほど密結合です。
...
以下、弱い順に同一仕様のコードを並べます。

※逆に、下から上に読み返すと、そのままリファクタリングの手順になります。
...
Checkout は冒頭の `契約結合` の節から、ダッシュボード向け読み取り用に `ICheckoutService.GetRecent(int days)` を公開しています。Dashboard は本来そこを通じて直近の `Receipt` を受け取り、利用側で合計すれば済む話です。

しかし「いちいち全件取得して足し直すのは無駄」「集計が必要になるたびに Checkout チームに新しいエンドポイントを追加してもらうのを待っていられない」といった理由で、Dashboard モジュールが Checkout の内部テーブルを直接 SELECT し始めた状態がこれです。
...
// Dashboardモジュール：Checkout.GetRecent を使わず、"internal" を直接覗く
namespace Dashboard;

public class SalesDashboard(IDbConnection db)
{
    // 本来は ICheckoutService.GetRecent(30) を呼んで合計すれば済む
    public decimal GetRevenueThisMonth() =>
        db.ExecuteScalar<decimal>(@"
            SELECT SUM(Total) FROM checkout.Receipts
...
| --- | --- | --- |
| Dashboard が内部DBを直読するのをやめ、既存の `GetRecent` 経由に戻す | 内部実装への侵入を契約の向こう側に戻す | 侵入結合 → 機能結合（重複） |
